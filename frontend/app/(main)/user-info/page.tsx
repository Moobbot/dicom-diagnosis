...
import { getMe, updateUser, changeAvatar } from '@/app/api/UserApi';
...
const AccountInfo: React.FC = () => {
    ...
    const [avatar, setAvatar] = useState<string>('https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y');
    ...
    const fetchUserData = async (id: string): Promise<void> => {
        try {
            ...
            if (response) {
                ...
                setAvatar(response.data.detail_user?.avatar || 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y');
            }
        } catch (error) {
            ...
            if (toast.current) {
                toast.current.show({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'Cannot load user information',
                    life: 3000
                });
            }
        }
    ...
    const handleSubmit = async (): Promise<void> => {
        try {
            ...
            if (response.status) {
                showToast('success', 'Success', response.message || 'Update information successfully');
                fetchUserData(userId);
                closeUpdateDialog();
            } else {
                throw new Error(response.message || 'Update information failed');
            }
        } catch (error: any) {
            console.error('Error updating profile:', error);
            showToast('error', 'Error', `Cannot update information: ${error.message}`);
        }
    };
    ...
    const handleAvatarChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > MAX_FILE_SIZE) {
            showToast('error', 'Error', 'File size cannot exceed 5MB');
            return;
        }
        ...
        if (!hasValidExtension) {
            showToast('error', 'Error', 'Only PNG, JPG or JPEG files are accepted');
            return;
        }
